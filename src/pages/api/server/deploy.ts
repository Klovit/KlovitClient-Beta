...
if (srvram || srvdisk || srvcpu || srvegg || srvlocation) {
  try {
    ...
    if (servers2 >= usrpackage.servers + extra.servers) {
      return redirect(`/server/deploy?error=You have reached your package's server creation limit.`);
    }
    let ram3;
    let disk3;
    let cpu3
    ...
    if (name.length < 1) { 
      return redirect(`/server/deploy?error=The length of server's name is less than 1`);
    }
    if (name.length > 191) {
      return redirect(`/server/deploy?error=The length of server's name is more than 191`);
    }
  
    let location = srvlocation;
  
    if (Object.entries(config.locations).filter(vname => vname[0] == location).length !== 1) {
      return redirect(`server/deploy?error=Invalid Location`);
    }
    let egginfo = config.eggs[srvegg];
    if (!config.eggs[srvegg]) {
      return redirect(`/server/deploy?error=Invalid Egg`);
    }
    let ram;
    if (config.resource_type === "GB") {
    ...
    if (!isNaN(ram) && !isNaN(disk) && !isNaN(cpu)) {
      if (+ram3 + +srvram > +usrpackage.ram + +extra.ram) {
        return redirect(`/server/deploy?error=The RAM given exceeds your Package's RAM limit.`);
      }
      if (+disk3 + +srvdisk > +usrpackage.disk + +extra.disk) {
        return redirect(`/server/deploy?error=The Disk given exceeds your Package's Disk limit.`);
      }
      if (+cpu3 + +srvcpu > +usrpackage.cpu + +extra.cpu) {
        return redirect(`/server/deploy?error=The CPU given exceeds your Package's CPU limit.`);
      }
      if (egginfo.limits.minimum.ram) if (srvram < egginfo.limits.minimum.ram) {
        return redirect(`/server/deploy?error=Given RAM is below the minimum limit.&limit=${egginfo.limits.minimum.ram}`);
      }
      if (egginfo.limits.minimum.disk) if (srvdisk < egginfo.limits.minimum.disk) {
        return redirect(`/server/deploy?error=Given Disk is below the minimum limit.&limit=${egginfo.limits.minimum.disk}`);
      }
      if (egginfo.limits.minimum.cpu) if (srvcpu < egginfo.limits.minimum.cpu) {
        return redirect(`/server/deploy?error=Given CPU is below the minimum limit.&limit=${egginfo.limits.minimum.cpu}`);
      }
      if (egginfo.limits.maximum) {
        if (egginfo.limits.maximum.ram) if (srvram > egginfo.limits.maximum.ram) {
          return redirect(`/server/deploy?error=Given CPU is above the maximum limit.&num=${egginfo.limits.maximum.ram}`);
        }
        if (egginfo.limits.maximum.disk) if (srvdisk > egginfo.limits.maximum.disk) {
          return redirect(`/server/deploy?error=Given CPU is above the maximum limit.&num=${egginfo.limits.maximum.disk}`);
        }
        if (egginfo.limits.maximum.cpu) if (srvcpu > egginfo.limits.maximum.cpu) {
          return redirect(`/server/deploy?error=Given CPU is above the maximum limit.&num=${egginfo.limits.maximum.cpu}`)
        }
      }
      ...
      if (serverinfo.statusText !== "Created") {
        ...
        return redirect(`/server/deploy?error=Error while creating your server.`);
      }
      ...
      const serverId = serverinfotext.attributes.id;
      const serverUuid = serverinfotext.attributes.uuid;
      const retrivedram = serverinfotext.attributes.limits.memory;
      const retriveddisk = serverinfotext.attributes.limits.disk;
      const retrivedcpu = serverinfotext.attributes.limits.cpu;
      ...
      const oldservers = usrinfo.servers || {};
      const updatedServers = {
        ...oldservers,
        ...newServer
      };
      const newinfo = {
        package: usrinfo.package,
        balance: usrinfo.balance,
        permission: usrinfo.permssion,
        invoices: usrinfo.invoices,
        tickets: usrinfo.tickets,
        ...
        extraresources: {
          ...
          disk: usrinfo.disk,
          cpu: usrinfo.cpu,
          servers: usrinfo.servers
        }
      }
      await db.set("user-" + email, newinfo)
      return redirect("/?success=Created your Server.");
    }
  } 
  catch(err) {
    console.log(err)
    return redirect(`/server/deploy?error=${err}`);
  }
  } else {
    return redirect(`/server/deploy?error=A Field is missing.`);
  }
}
